...
class Database:
    ...
    def migrate(self) -> None:
        ...
        # additive migration for databases created before `note` existed
        # (CREATE TABLE IF NOT EXISTS never adds a column to a table already present).
        have = {row["name"] for row in self.conn.execute("PRAGMA table_info(groups)")}
        if "note" not in have:
            self.conn.execute("ALTER TABLE groups ADD COLUMN note TEXT")
        self.conn.commit()
