...
def groups_text(db: Database, note: str | None = None) -> str:
    where = "WHERE g.note LIKE ?" if note else ""
    params = (f"%{note}%",) if note else ()
    rows = db.conn.execute(
        f"""
        SELECT g.group_id, g.label, g.note, COUNT(u.unit_id) AS units,
    ...
    lines = ["GROUP\tLABEL\tNOTE\tUNITS\tRELEASED\tREMOTE\tINVALID"]
    for row in rows:
        lines.append(f"{public_group_id(row['group_id'])}\t{row['label'] or ''}\t{row['note'] or ''}\t{row['units']}\t{row['released']}\t{row['remote_visible']}\t{row['invalid_held']}")
    return "\n".join(lines)
