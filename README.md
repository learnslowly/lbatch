...
Annotate a submission with a free-text note, then filter groups by it (handy for finding
related jobs in a large pool):

```bash
lbatch --lbatch-note="genome-wide chr19" job/scan.batch   # note is stored on the group
lbatch status --note genome                                # only groups whose note contains "genome"
```
